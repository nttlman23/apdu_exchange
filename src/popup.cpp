...
void PopUp::show()
{
    ...
    setGeometry(QApplication::desktop()->availableGeometry().width() - 5 - width() +
                QApplication::desktop()->availableGeometry().x(),
                QApplication::desktop()->availableGeometry().height() - 5 - height() +
...
void PopUp::show(int screen)
{
    ...
    qDebug() << "Screen number: " << screen;
    setGeometry(QApplication::desktop()->availableGeometry(screen).width() - 36 - width() +
                QApplication::desktop()->availableGeometry(screen).x(),
                QApplication::desktop()->availableGeometry(screen).height() - 36 - height() +
                QApplication::desktop()->availableGeometry(screen).y(),
                width(),
                height());
    QWidget::show();                // Отображаем виджет, который полностью прозрачен
